...
WordList = {}
WordList.__index = WordList

function WordList:new(...)
  return setmetatable({...}, self)
end

function WordList:pick()
  return self[math.random(1, #self)]
end

adjectives1 = WordList:new("artless", "bawdy", "beslubbering", "bootless", "brazen",
...
adjectives2 = WordList:new("base-court", "bat-fowling", "beef-witted",
...
nouns = WordList:new("apple-john", "baggage", "barnacle", "bladder", "boar-pig",
...
function curse()
  local adjective1 = adjectives1:pick()
  local adjective2 = adjectives2:pick()
  local noun = nouns:pick()
  return string.format('Thou %s %s %s!', adjective1, adjective2, noun)
end
...
if #arg > 0 then
  if arg[1] == '-h' then
    print(string.format(
      'Usage: %s [-h] [n] (n is the number of curses you want)', arg[0]))
    os.exit(false)
  end
  num = math.tointeger(arg[1])
  if not num then
    io.stderr:write('Not a valid number: ', arg[1], '\n')
    os.exit(false)
  end
else
  repeat
    io.write('Number of curses: ')
    num = math.tointeger(io.read('l'))
  until num
end
